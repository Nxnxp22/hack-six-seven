...
import { fetchStability, applyStabilityDelta, fetchCoins, applyCoins } from "./powerOverload/services/stabilityApi";
...
// Decay rates are now enforced server-side; kept here only as reference comments.
// powerOverload: -1% / 5s | reactionFailure: -1% / 7s
// commCollapse: -1% / 10s | securityLockdown: -1% / 14s
...
const INITIAL_MODULES: GameModule[] = [
  {
    ...
    name: "POWER OVERLOAD",
    description:
      "Reroute the power grid before critical systems fail. Cut the correct wires to prevent cascade failure.",
    stability: 100,
  ...
  },
  {
    ...
    description:
      "Reactor cooling sequence compromised. Replicate the pattern to stabilize reactor core temperature.",
    stability: 100,
  ...
  },
  {
    ...
    name: "COMM COLLAPSE",
    description:
      "Emergency transmission detected. Decode the morse signal to receive critical survivor coordinates.",
    stability: 100,
  ...
  },
  {
    ...
    description:
      "System lockdown initiated. Analyze security logs to find the override code and restore access.",
    stability: 100,
...
// sessionStorage/setInterval stability removed — backend now owns stability state.
...
const MainDashboard: React.FC = () => {
  ...
  // ── Sync stability + coins from backend ───────────────────────────────
  const syncStability = useCallback(async () => {
    try {
      const [{ modules: serverModules }, { balance }] = await Promise.all([
        fetchStability(),
        fetchCoins(),
      ]);
      setModules((prev) =>
        prev.map((m) => {
          const found = serverModules.find((s) => s.moduleId === m.id);
          return found ? { ...m, stability: found.stability } : m;
        }),
      );
      setCoins(balance);
    } catch {
      // Keep current values if the request fails
    }
  }, []);

  // Fetch immediately on mount, then every 5 seconds (matches fastest decay rate)
  useEffect(() => {
    syncStability();
    const interval = setInterval(syncStability, 5_000);
  ...
  useEffect(() => {
    ...
    // Prevent duplicate processing of the same location state object (e.g. on StrictMode double-mount or re-renders)
    if (processedStateRef.current === state) return;
    processedStateRef.current = state;

    if (state.stabilityGained !== undefined && state.feature) {
      // Apply gain to backend, then re-sync local state
      applyStabilityDelta(state.feature, state.stabilityGained).then((updated) => {
        setModules((prev) =>
          prev.map((m) =>
            m.id === state.feature ? { ...m, stability: updated.stability } : m,
          ),
        );
      });

      const coinsGained = state.coinsGained ?? 0;
      if (coinsGained > 0) {
        applyCoins(coinsGained).then(({ balance }) => setCoins(balance));
        setFlashMsg({
          text: `✓ MAINTENANCE SUCCESS — STABILITY +${state.stabilityGained}%, COINS +${coinsGained} 🪙`,
          type: "success",
        });
      } else {
        setFlashMsg({
          text: `✓ MAINTENANCE SUCCESS — STABILITY +${state.stabilityGained}%`,
          type: "success",
        });
      }
    } else if (state.stabilityLost !== undefined && state.feature) {
      // Apply loss to backend, then re-sync local state
      applyStabilityDelta(state.feature, -state.stabilityLost).then((updated) => {
        setModules((prev) =>
          prev.map((m) =>
            m.id === state.feature ? { ...m, stability: updated.stability } : m,
          ),
      ...
      setFlashMsg({
        text: `✗ CRITICAL FAILURE — STABILITY −${state.stabilityLost}%`,
        type: "fail",
      });
    }
    ...
    <div className="min-h-screen bg-black text-white font-mono relative overflow-hidden select-none">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          ...
          style={{
            top: `${scanlineY}%`,
            background:
              "linear-gradient(90deg, transparent, #e91e8c 40%, #e91e8c 60%, transparent)",
          }}
        />
      </div>
      ...
      <header className="relative z-20 border-b border-zinc-800/60 bg-black/90 backdrop-blur-sm px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div
            className="w-9 h-9 rounded border border-[#e91e8c]/70 bg-[#e91e8c]/10 flex items-center justify-center flex-shrink-0"
            style={{ boxShadow: "0 0 12px rgba(233,30,140,0.25)" }}
          >
            <svg
              viewBox="0 0 24 24"
              className="w-5 h-5 fill-none stroke-[#e91e8c]"
              strokeWidth="2"
            >
              <path
                strokeLinecap="round"
        ...
        <div className="flex items-center gap-6">
          <div className="text-right">
            <div className="text-zinc-500 text-[9px] tracking-widest uppercase">
              COINS
            </div>
            <div className="text-lg font-black tracking-wider text-yellow-500 text-glow-yellow flex items-center justify-end gap-1 select-none">
              <span className="animate-pulse">🪙</span> {coins}
            </div>
          </div>
          <div className="text-right">
            <div className="text-zinc-500 text-[9px] tracking-widest uppercase">
            ...
            <div
              ...
              style={{
                textShadow: systemCritical
                  ? "0 0 15px rgba(239,68,68,0.7)"
                  : "0 0 15px rgba(233,30,140,0.6)",
              }}
            >
              {systemStability}%
            </div>
      ...
      <div className="relative z-20 border-b border-zinc-800/40 px-6 py-3 bg-black/70">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-2 text-[10px] tracking-widest uppercase">
            <span
              className={`w-1.5 h-1.5 rounded-full animate-pulse ${systemCritical ? "bg-red-500" : "bg-[#e91e8c]"}`}
            />
            <span className="text-zinc-500">SYSTEM STABILITY:</span>
            <span
              className={`font-bold ${systemCritical ? "text-red-400 stability-critical-blink" : "text-[#e91e8c]"}`}
            >
              {systemStability}%
            </span>
            <span className="text-zinc-700 text-[9px] hidden sm:inline">
              (avg · 4 modules)
            </span>
          </div>
          <div
            className={`text-[10px] tracking-widest uppercase ${systemCritical ? "text-red-400 stability-critical-blink" : "text-zinc-600"}`}
          >
            {systemCritical
              ? "CRITICAL · IMMINENT CASCADE FAILURE"
              : systemStability >= 70
                ? "SYSTEM SECURE · STABLE STATE"
                : systemStability >= 40
        ...
        <div className="h-2 bg-zinc-900 rounded-full overflow-hidden">
          <div
            ...
            style={{
              ...
              boxShadow: systemCritical
                ? "0 0 12px rgba(239,68,68,0.8)"
                : `0 0 10px ${NEXUS_PINK}b3`,
            }}
          />
        </div>
      </div>
      ...
      {flashMsg && (
        <div
          className={`relative z-30 px-6 py-2 text-xs font-bold tracking-widest text-center border-b transition-all
          ${
      ...
      <main className="relative z-10 p-6 max-w-5xl mx-auto">
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {/* Icon */}
                    <div
                      className={`w-10 h-10 rounded flex items-center justify-center flex-shrink-0 border border-current transition-all group-hover:shadow-lg ${mod.iconBgClass}`}
                      style={{ boxShadow: `0 0 14px ${mod.accentHex}40` }}
                    >
                      <div className={mod.accentClass}>{mod.icon}</div>
                    </div>
                    <div>
                      <div className="text-zinc-500 text-[9px] tracking-[0.25em] uppercase mb-0.5">
                        {mod.category}
                      </div>
                      <div
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <span
                      className={`text-[10px] font-bold ${stab.text} ${stab.blink ? "stability-critical-blink" : ""}`}
                    >
                ...
                {/* Corner decoration */}
                <div
                  className="absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 rounded-tl-lg opacity-60 transition-opacity group-hover:opacity-100"
                  style={{ borderColor: mod.accentHex }}
                />
                <div
                  className="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 rounded-br-lg opacity-60 transition-opacity group-hover:opacity-100"
                  style={{ borderColor: mod.accentHex }}
                />
              </div>
            );
        ...
        <div className="mt-6 flex items-center justify-between text-[9px] tracking-widest text-zinc-700 uppercase">
          ...
          <div className="flex items-center gap-4">
            <span
              className={`flex items-center gap-1 ${systemCritical ? "text-red-400 stability-critical-blink" : "text-[#e91e8c]"}`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full animate-pulse ${systemCritical ? "bg-red-500" : "bg-[#e91e8c]"}`}
              />
