...
const PowerOverloadHardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchManualRules('HARD').then(setRules).catch(console.error)
  ...
  const renderRuleText = (text: string) =>
    text.split(' ').map((word, idx) => {
      ...
      if (w === 'CYAN') return <span key={idx} className="text-cyan-400 font-semibold mx-0.5">{word}</span>
  ...
  const renderInstruction = (instruction: string) => {
    const colors = new Set(['GREEN', 'YELLOW', 'CYAN', 'RED', 'BLUE', 'ORANGE', 'PURPLE'])
    return instruction.split(' ').map((word, i) => {
      const w = word.replace(/[.,/#!$%^&*;:{}=\-_`~()]/g, '').toUpperCase()
      if (colors.has(w) || w.startsWith('#') || word.includes('-'))
        return <span key={i} className="text-yellow-500 text-xl font-black mx-1.5 uppercase text-glow-yellow">{word}</span>
  ...
  const handleWireCut = async (wireId: string) => {
    ...
    } catch (err) { console.error('Error cutting wire:', err) }
  }
  ...
  if (!game)
    return <div className="bg-black min-h-screen text-zinc-500 font-mono flex items-center justify-center">LOADING HARD DATA MODULES...</div>
        ...
        {isManualOpen && (
          <div className="border border-yellow-500 bg-zinc-950/90 rounded-lg p-6 mb-8 max-w-2xl w-full mx-auto font-mono text-[11px] text-zinc-500 border-glow-yellow animate-fadeIn leading-relaxed">
            <div className="flex items-center gap-2 border-b border-zinc-900 pb-3 mb-4">
              ...
              <h3 className="text-yellow-500 font-bold text-xs tracking-wider uppercase">NEXUS HACKING MANUAL : POWER OVERLOAD [HARD]</h3>
            </div>
            ...
            <div>
              <p className="text-yellow-500 font-bold mb-1.5 uppercase tracking-wide">⚡ HARD RESOLUTION PROTOCOL (5 wires — cut 3)</p>
              <ul className="space-y-1.5 pl-4 list-disc">
                ...
                <li className="list-none text-zinc-500 font-mono text-[9px] mt-2 uppercase leading-normal tracking-wide border-t border-zinc-900 pt-2">
                  ⚠️ System Signatures: GREEN (<span className="text-emerald-500">BIO-SYNAPSE</span>), YELLOW (<span className="text-amber-400">SOLAR-VOLT</span>), CYAN (<span className="text-cyan-400">CRYO-LINK</span>), RED (<span className="text-rose-500">DANGER-CORE</span>), PURPLE (<span className="text-purple-500">VOID-NODE</span>)
                </li>
              </ul>
            </div>
          </div>
