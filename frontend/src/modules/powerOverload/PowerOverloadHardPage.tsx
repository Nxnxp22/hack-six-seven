    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.isGameOver) {
          alert(response.message || "SUCCESSFULLY DEFUSED! Stability restored: +40%");
          navigate("/", { state: { stabilityGained: 40, status: "SUCCESS", feature: "powerOverload" } });
        }
      } else {
              ...
              <div>
                ...
                <ul className="space-y-1.5 pl-4 list-disc">
                  ...
                  <li className="list-none text-zinc-500 font-mono text-[9px] mt-2 uppercase leading-normal tracking-wide border-t border-zinc-900 pt-2">
                    ⚠️ System Signatures Legend: GREEN (<span className="text-emerald-500 font-semibold">BIO-SYNAPSE</span>), YELLOW (<span className="text-amber-400 font-semibold">SOLAR-VOLT</span>), CYAN (<span className="text-cyan-400 font-semibold">CRYO-LINK</span>), RED (<span className="text-rose-500 font-semibold">DANGER-CORE</span>), PURPLE (<span className="text-purple-500 font-semibold">VOID-NODE</span>).
                  </li>
                </ul>
              </div>
            </div>
