...
const PowerOverloadEasyPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchNewGame('EASY', restoreId)
      .then((data) => {
        ...
        if (isRestored) {
          ...
          if (remaining <= 0) {
            // Timer expired while away — reset it rather than immediately failing
    ...
    fetchManualRules('EASY').then(setRules).catch(console.error)
  ...
  const renderInstruction = (instruction: string) => {
    const colors = ['GREEN', 'YELLOW', 'CYAN', 'RED', 'BLUE', 'ORANGE', 'PURPLE']
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.isGameOver) {
          ...
          setResult({ success: true, stabilityChange: 10, coinsChange, timeTaken })
        }
      } else {
        sessionStorage.removeItem('active_game_session_id')
        sessionStorage.removeItem('active_game_difficulty')
  ...
  if (!game)
    return <div className="bg-black min-h-screen text-zinc-500 font-mono flex items-center justify-center">LOADING EASY DATA MODULES...</div>
        ...
        {isManualOpen && (
          <div className="border border-yellow-500 bg-zinc-950/90 rounded-lg p-6 mb-8 max-w-2xl w-full mx-auto font-mono text-[11px] text-zinc-500 border-glow-yellow animate-fadeIn leading-relaxed">
            <div className="flex items-center gap-2 border-b border-zinc-900 pb-3 mb-4">
              ...
              <h3 className="text-yellow-500 font-bold text-xs tracking-wider uppercase">NEXUS HACKING MANUAL : POWER OVERLOAD [EASY]</h3>
            </div>
            ...
            <div>
              <p className="text-yellow-500 font-bold mb-1.5 uppercase tracking-wide">⚡ EASY RESOLUTION PROTOCOL (3 wires)</p>
              <ul className="space-y-0.5 pl-4 list-disc">
                {rules.map((rule) => <li key={rule.id}>Rule {rule.rule_number}: {renderRuleText(rule.description)}</li>)}
              </ul>
            </div>
          </div>
        )}
...
export default PowerOverloadEasyPage
